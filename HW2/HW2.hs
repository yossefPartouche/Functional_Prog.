...
exprToString' :: Expr -> String
exprToString' expr = fst (exprToStringWithPrec expr)
  where
    exprToStringWithPrec :: Expr -> (String, Int)
    exprToStringWithPrec = \case
      Iden x -> (x, 3)  
      Lit n -> (show n, 3)
      Plus e1 e2 -> binaryOpToString 1 "+" e1 e2 False
      Minus e1 e2 -> binaryOpToString 1 "-" e1 e2 True
      Mul e1 e2 -> binaryOpToString 2 "*" e1 e2 False
      Div e1 e2 -> binaryOpToString 2 "/" e1 e2 True
    
    binaryOpToString :: Int -> String -> Expr -> Expr -> Bool -> (String, Int)
    binaryOpToString precedence op e1 e2 nonAssoc = 
      let (s1, p1) = exprToStringWithPrec e1
          (s2, p2) = exprToStringWithPrec e2
          -- Add parentheses if the sub-expression has lower precedence
          -- or if it's the right operand of a non-associative operator with same precedence
          needParens1 = p1 < precedence
          needParens2 = p2 < precedence || (p2 == precedence && nonAssoc)
          str1 = if needParens1 then "(" ++ s1 ++ ")" else s1
          str2 = if needParens2 then "(" ++ s2 ++ ")" else s2
      in (str1 ++ " " ++ op ++ " " ++ str2, precedence)
