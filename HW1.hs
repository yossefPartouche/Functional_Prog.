...
-- Is it possible to have such a generic function that you can apply with string --> bool and also Int --> Double???
impossible :: a -> b
...
takeFromGen :: Integer -> Generator a -> [a]
takeFromGen 0 _ = []
takeFromGen n (f, p, val)
    | not (p val) = []
    | otherwise = let next = f val
                 in next : takeFromGen (n-1) (f, p, next)
...
divisors' :: Integer -> Generator Integer
divisors' n = (nextDiv, (< abs' n), 0)
    where 
        nextDiv a = 
            let next = findNext (a +1)
            in if next >= abs' n then abs' n else next
        findNext k 
            | k >= abs' n = abs' n
            | abs' n `mod` k == 0 = k
            | otherwise = findNext $ k +1
-----------------------------------
